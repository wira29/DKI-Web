...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const articleItem = await prisma.post.findUnique({ where: { slug } });
  
  if (!articleItem) return { title: 'Artikel Tidak Ditemukan' };

  return {
    title: articleItem.title,
    description: articleItem.short_description,
    openGraph: {
      title: articleItem.title,
      description: articleItem.short_description,
      images: [articleItem.image],
    }
  };
}
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FAFAFA]">
        <div className="text-center">
          ...
          <Link href="/articles" className="text-primary hover:text-primary-hover">Kembali ke daftar artikel</Link>
        </div>
      </div>
    );
  }

  const dateFormatted = articleItem.published_at 
    ? new Date(articleItem.published_at).toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }) 
    : '';
    ...
    <main className="min-h-screen bg-[#FAFAFA] text-black">
      ...
      <section className="max-w-4xl mx-auto px-6 -mt-32 relative z-20 pb-32">
        <div className="relative rounded-[2.5rem] overflow-hidden aspect-[16/9] md:aspect-[21/9] bg-gray-100 mb-10 w-full shadow-2xl border border-white/10">
          ...
          <Link href="/articles" className="absolute top-6 left-6 inline-flex items-center text-sm font-medium text-white hover:text-white/80 transition-colors bg-black/40 backdrop-blur-md px-4 py-2 rounded-full border border-white/20">
        ...
        <div 
          ...
          dangerouslySetInnerHTML={{ __html: articleItem.short_description || '' }}
        />
      </section>
    </main>
  );
