      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="mb-12 max-w-4xl">
          <div className="inline-block px-3 py-1 mb-6 rounded-full border border-gray-200 text-xs font-semibold text-gray-500 uppercase tracking-widest">
            {event.type === 'ARTICLE' ? 'ARTIKEL / BERITA' : `${event.location_type || ''} EVENT`}
          </div>
          <h1 className="text-4xl md:text-5xl font-semibold text-black tracking-tight mb-6 leading-tight">
            {event.title}
          </h1>
          {event.event_date && event.type === 'ARTICLE' && (
            <div className="text-sm text-gray-400 mb-6">
              Dipublikasikan pada {dateFormatted}
            </div>
          )}
        </div>
        ...
        <div className={`grid grid-cols-1 ${event.type === 'EVENT' ? 'lg:grid-cols-3 gap-16' : 'lg:grid-cols-1 max-w-4xl'}`}>
          <div className={`${event.type === 'EVENT' ? 'lg:col-span-2' : ''} space-y-16`}>
            <section>
              <div 
                className="text-lg text-gray-600 leading-relaxed font-light space-y-6 prose prose-lg max-w-none"
